...
// import { poiSelectors } from './poi_outdated';
...
//     // TODO: это нужно переделать поскольку тот же квест может быть глубоко внитри вложенных POI
//     // Возможно нужно искать по всем POI в ячейке и их суб-POI рекурсивно
//     const cellPois = poiSelectors.selectPoisByParentId(cellId)(state);

//     if (cellPois?.length) {
//       for (const poi of cellPois) {
//         const priority = POI_ICON_PRIORITY[poi.type as PoiType];
...
//         cell.explorationLevel = Math.max(cell.explorationLevel, explorationLevel);
...
//       // теперь — просто parentId = cellId
//       get().pois.actions.explorePoisIn(cellId, explorationLevel);
//     },

//     modifyCellStatus: (cellId, progressKey, delta) =>
//       set((state) => {
//         const cell = state.map.cells[cellId];
